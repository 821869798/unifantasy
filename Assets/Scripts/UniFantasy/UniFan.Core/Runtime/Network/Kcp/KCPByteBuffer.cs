...
{
    class KCPByteBuffer : ICloneable
    {
        //字节缓存区
        private byte[] buf;
        //读取索引
        ...
        //对象池
        private static List<KCPByteBuffer> pool = new List<KCPByteBuffer>();
        ...
        private KCPByteBuffer(int capacity)
        {
        ...
        private KCPByteBuffer(byte[] bytes)
        {
        ...
        public static KCPByteBuffer Allocate(int capacity, bool fromPool = false)
        {
            if (!fromPool)
            {
                return new KCPByteBuffer(capacity);
            }
            lock (pool)
            {
                KCPByteBuffer bbuf;
                if (pool.Count == 0)
                {
                    bbuf = new KCPByteBuffer(capacity)
                    {
                        isPool = true
                    };
                    return bbuf;
        ...
        /// </param>
        /// <returns>ByteBuffer对象</returns>
        public static KCPByteBuffer Allocate(byte[] bytes, bool fromPool = false)
        {
            if (!fromPool)
            {
                return new KCPByteBuffer(bytes);
            }
            lock (pool)
            {
                KCPByteBuffer bbuf;
        ...
        public void Write(KCPByteBuffer buffer)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public KCPByteBuffer Copy()
        {
            if (buf == null)
            {
                return new KCPByteBuffer(16);
            }
            if (readIndex < writeIndex)
            {
                byte[] newbytes = new byte[writeIndex - readIndex];
                Array.Copy(buf, readIndex, newbytes, 0, newbytes.Length);
                KCPByteBuffer buffer = new KCPByteBuffer(newbytes.Length);
        ...
        public object Clone()
        {
            ...
            KCPByteBuffer newBuf = new KCPByteBuffer(buf)
            {
